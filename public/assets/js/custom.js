  ...
  $(document).on("click", ".friend-select input", function () {
    ...
    if (content.is(":checked")) {
      ...
      content
        .parent()
        .siblings()
        .children(".custom-radio-sel-friend")
        .addClass("active");
    } else {
      content
        .parent()
...
$(document).on("click", ".rating-sec", function () {
  ...
  $(".rating-sec").each(function () {
    ...
    const imgSrc = $(this).hasClass("active")
      ? "assets/svg/white-star.svg"
      : "assets/svg/grey-star.svg";
...
function maskNumber() {
  ...
  let maskedNumber =
    maskedPart.replace(/(.{4})/g, "$1 ").trim() + " " + lastPart;
